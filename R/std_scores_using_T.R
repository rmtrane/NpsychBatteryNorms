...
std_scores_using_T <- function(
  ...
  stopifnot("T-scores not available for 'var_name'. See 'names(t_score_coefs)' for available variables" = var_name %in% names(t_score_coefs))
  ...
  stopifnot("'sex' must either be a character vector of 'f' and 'm', or a numeric
            vector of 1 (male) and 0 (female)" = (is.numeric(sex) & all(unique(sex) %in% c(0, 1, NA))) |
  ...
  stopifnot("'raw_scores', 'sex', 'education', and 'age' must all have the same length" =
              length(unique(c(length(raw_scores), length(age), length(education), length(sex)))) == 1)
    ...
    T_scores <- 50 + (
      (
        (
          ...
          coefs_to_use[["intercept3"]] + age^2 * coefs_to_use[["age^2_2"]] +
